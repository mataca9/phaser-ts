...
export class Hero {
    ...
    constructor(scene: Phaser.Scene, id?: string) {
        ...
        this.scene = scene;
        this.sprite = this.scene.physics.add.sprite(getGameWidth(this.scene) / 2, getGameHeight(this.scene) / 2, 'hero')
            .setBounce(1, 1)
    ...
    shoot() {
        ...
        this.scene.physics.moveTo(bullet, this.scene.input.x, this.scene.input.y, this.bulletSpeed);
        setTimeout(() => {
            bullet.destroy();
        }, this.bulletLifespan);
    }
    ...
    addHearts() {
        for(let i = 0; i < this.hp; i++) {
            this.hearts.push(this.scene.add.sprite(getGameWidth(this.scene) / 2 - 380 + (i * 20), getGameHeight(this.scene) / 2 - 280, 'heart'));
        }
    }
